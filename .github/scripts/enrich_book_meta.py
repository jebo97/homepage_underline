...
# 특정 제목은 매칭 로직과 무관하게 지정한 네이버 카탈로그로 강제 고정한다(값=카탈로그 ID).
# 자동 매칭이 못 잡거나 잘못 잡는 '예외'만 둔다. 새 항목은 아래 사유 그룹에 맞춰 추가.
NAVER_FORCE_CATALOG = {
    # ── 전문서점 원판 우선 (일반몰 전용 신판·굿즈판이 최신이라 먼저 잡히는 것 교정) ──
    "1Q84 1": "32485940318",                    # 신판 대신 전문서점판(문학동네 2009 원판)
    "82년생 김지영": "32467074651",              # 워터프루프북 대신 원판(민음사 2016)
    "서랍에 저녁을 넣어 두었다": "32463129802",   # 일반몰 신판 대신 전문서점판(문학과지성사 2013)
    "설민석의 삼국지 1": "32455467972",           # 라이트 에디션 대신 원판(세계사 2019)
    "세이노의 가르침": "37313533623",             # 보급판 대신 원판(데이원 2023)
    "소년이 온다": "32491401626",                # 일반몰 신판 대신 전문서점판(창비 2014 원판)

    # ── 현재 판매판/개정판 우선 (절판 구판이 먼저 잡히는 것 교정) ──
    "구해줘": "32436136952",                     # 절판 2010판 대신 현재판(밝은세상 2022)
    "나의 한국현대사": "32487158133",             # 절판 원판 대신 개정증보판(돌베개, 1959-2020)
    "녹나무의 파수꾼": "49023649618",             # 2020 원판 대신 2024 무선제본특별판
    "여행의 이유": "46931760625",                # 문학동네 구판 대신 현재 출판사판(복복서가 2024)
    "위험한 비너스": "49366169643",              # 제목·출판사 바뀐 개정판(하빌리스, '아름답고 위험한 이름, 비너스')

    # ── 오매칭 방지 (각색·성인판 등 다른 책이 잡히는 것 교정) ──
    "마시멜로 이야기(어린이를 위한)": "32493599085",   # 성인판 대신 어린이판(깊은책속옹달샘)
    "베르나르 베르베르의 상상력 사전": "32484721809",  # 만화/아동 각색판 대신 텍스트 원서(열린책들)

    # ── 권번호 파싱 불가 → 직접 지정 ──
    "원피스 110: 시대의 일렁임": "52466229260",   # 카탈로그 제목 '원피스 ONE PIECE 110'

    # ── 개정마다 제목이 바뀐 시리즈 → 현재판으로 통합 (제목 자체는 TITLE_OVERRIDES 로 통일) ──
    "해커스 토익 기출 VOCA(보카)": "32436333110",
    "해커스 토익 RC Reading(리딩) 기본서": "36347622619",
    "해커스 토익 LC Listening(리스닝) 기본서": "36350329619",
    "해커스 토익 스타트 RC Reading (리딩) 입문서": "32467685597",
}
...
NAVER_SKIP_TITLES = {
    ...
    "아름다운 마무리",
    "인연 이야기",
    "보다",  # 김영하 산문(절판, 현재 중고만) — 동명의 다른 저자 책으로 오매칭되어 비움
}
