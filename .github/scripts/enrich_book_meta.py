...
NAVER_FORCE_CATALOG = {
    ...
    "해커스 토익 기출 VOCA(보카)": "32436333110",  # 개정마다 제목이 바뀐 시리즈 통합 → 현재판
    "해커스 토익 RC Reading(리딩) 기본서": "36347622619",  # 개정마다 제목이 바뀐 Reading 기본서 통합 → 현재판
    "해커스 토익 LC Listening(리스닝) 기본서": "36350329619",  # 개정마다 제목이 바뀐 Listening 기본서 통합 → 현재판
    "해커스 토익 스타트 RC Reading (리딩) 입문서": "32467685597",  # 스타트(입문) Reading 시리즈 통합 → 현재판
    "82년생 김지영": "32467074651",  # 워터프루프/특별판 대신 원판(민음사 2016)
}
